...
if ( is_admin() ) {
   $request = new Goo_testimonial_meta();
}



class Goo_testimonial_meta{
...
*/
	public function add_meta( $post_type ){
		...
		$post_types = array( 'testimonial' );     // limit to certain post types
...
*/
	public function render_meta( $post ){
		...
		/*
			...
			*
		*/
		$_pre_test_attr = get_post_meta( $post->ID, '_goo_testimonial_attribution', true );
		...
		*/
		?>
		<label for="_goo_testimonial_attribution"><?php _e( 'Location of review' ); ?></label> <br/><br/>
		<input type="text" id="_goo_testimonial_attribution" name="_goo_testimonial_attribution" style="width: 100%; resize: none;" value="<?php echo esc_attr( $_pre_test_attr ); ?>" /><br/><br/>
